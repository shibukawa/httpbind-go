...
---
Host-side generator analysis must identify net/http and httpbinder symbols by resolved type identity (go/types), not bare selector names.
...
problem:
  ...
  - error constructors fail recognition when httpbinder is imported under an alias
approach: rule:go-types-symbol-identity
configuration:
  default: net/http and httpbinder identities
